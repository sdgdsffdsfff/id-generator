package com.wenjifeng.idgenerator.controller.redis;
...
/**
 * @Description 基于redis方式生成id的测试controller
...
public class RedisIdController {
    ...
    public Long getSequence () {
        return RedisIdUtil.getId(RedisIdEnum.REDIS_ID);
    }
    ...
    public Long qps() {
        ...
        while(System.currentTimeMillis() - startTime < 1000) {
            RedisIdUtil.getId(RedisIdEnum.REDIS_ID);
            count++;
        }
        return count;
    }
    ...
    public String nums() {
        ...
        new Thread(()->{
            int i = 2000;
            while (i-- > 0) {
                Long id = RedisIdUtil.getId(RedisIdEnum.REDIS_ID);
    ...
    public void concurrent () {
        ...
        for (int i = 0;i<requestTotal;i++) {
            ...
            executorService.execute(()->{
                try {
                    semaphore.acquire();
                    Long result = RedisIdUtil.getId(RedisIdEnum.REDIS_ID);
